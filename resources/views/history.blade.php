...
<hr>   
    ...
    <li>Mutation: <a href="{{url('balances')}}/{{$balance->balance_code}}/{{$version->mutation->mutation_count}}" >{{$version->mutation_id}}</a> - At {{date('d-m-Y H:i:s', strtotime($version->updated_at))}} -  {{$version->updatetype}} By {{$version->editor->balances->where('id', $balance->id)->pluck('pivot.nickname')->first()}} 
